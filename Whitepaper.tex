...
% \usepackage[bookmarks=true, unicode=true, pdftitle={Cap9: A security
...
\title{Cap9: A security protocol and framework for smart
contracts}\label{cap9-a-security-protocol-and-framework-for-smart-contracts}
...
Cap9 is a minimal open-source exokernel\footnote{An exokernel exposes as much
of the underlying hardware the system as possible, without larger abstractions
such as filesystems. In Cap9 we expose all the underlying mechanisms (such as
...
organizations. Cap9 enables organizations that will grow in complexity and
will require long term extensibility.

Using Cap9, users can establish an organization on the blockchain with
a much higher level of confidence in its stability and security. The
safety model Cap9 provides not only reduces risk, but allows
organizations to be designed in a much more flexible and robust manner.

The exokernel model of Cap9 allows organizations to freely define
their own systems and procedures, but within a safe and controlled
environment. The Cap9 kernel provides the building blocks and
...
Cap9 provides, designers of these systems will then be able to
...
Cap9, AragonOS features a permission system and a kernel, however, after that
...
Cap9 is targeted at isolating and limiting \emph{code}, the permission system
...
analysis tools, and even some that follow a similar ideal to Cap9 and
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.49\textwidth]{media/Cap9KernelOverview}

    \caption{Similar to how existing operating systems act as an
    interface to hardware for applications on a computer, the Cap9 kernel
...
Cap9 also uses this system call mechanism, and the sequence of EVM opcodes
that define a Cap9 system call are outlined below. The
...
As an operating system Cap9 should have no say over what kind of
transactions and programs a system wants to execute, and should not impose any
format on those transactions. Cap9 follows an
exokernel design, where the kernel itself should stay out of the user's
code as much as possible.

We can divide the calls that a Cap9 kernel has to accept into two categories:
...
By using an operating system model, Cap9 is able to enforce isolation
...
Cap9 system will also be opaque black boxes. The difference is, this
...
applying isolation, Cap9 allows developers to separate their system
...
We established two critical components of the Cap9 exokernel:
...
To implement these policies, Cap9 uses a capability-based
...
\item
  Cap9 follows a microkernel approach so as to give as much freedom to
...
All policies in Cap9 are applied directly to procedures. Cap9 is a
...
For this reason, Cap9 aims to help system designers implement their
own higher level permissions systems on solid primitives, rather than to
anticipate all the use cases for those using Cap9.

Presented here is an outline of what a simplistic capability system
might look like in Cap9. A procedure is created in two steps:
...
The Cap9 exokernel protocol provides a secure environment for
organizations to establish extendible business logic through restricted
smart contracts defined as procedures.

By using a low level capability-based security model, Cap9 allows
